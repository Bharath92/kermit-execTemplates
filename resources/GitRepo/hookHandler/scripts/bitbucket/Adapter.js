...
Bitbucket.prototype.postDeployKey = function (owner, repo, key, title,
  callback) {
  var url = this.baseUrlv2 + '/repositories/' + owner + '/' + repo +
    '/deploy-keys';
...
Bitbucket.prototype.getCompareDiff = function (owner, repo, oldSha, newSha,
  callback) {
  var url = this.baseUrlv2 + '/repositories/' + owner + '/' + repo + '/diff/' +
    oldSha + '..' + newSha;
  this.get(url, {}, callback);
};
