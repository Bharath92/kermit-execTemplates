...
function getCommitDiffFiles(params, callback) {
  ...
  bag.who = util.format('gitlab|%s|projectFullName:%s|commitSha:%s',
...
function _checkInputParams(bag, next) {
  ...
  bag.gitlabAdapter = new GitLabAdapter(
    bag.params.providerIntegrationValues.token, bag.params.providerUrl);
  return next();
}
...
function _getFilePaths(bag, next) {
  ...
  bag.gitlabAdapter.getCompareDiff(sourceId, beforeCommitSha, commitSha,
    function (err, diff) {
      if (err)
      ...
      _.each(diff.diffs,
        function (thisDiff) {
          ...
          bag.fileList.push(thisDiff.old_path);
          if (thisDiff.new_path !== thisDiff.old_path)
            bag.fileList.push(thisDiff.new_path);
