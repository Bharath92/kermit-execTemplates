...
Adapter.prototype.getDeployKeys = function(projectKey, repoSlug, callback) {
  var url = util.format('%s/projects/%s/repos/%s/ssh',
    this.v1KeysPrefix, projectKey, repoSlug);
  this.get(url, {}, callback);
};

Adapter.prototype.postDeployKey = function(projectKey, repoSlug, label,
  publicKey, callback) {
  var url = util.format('%s/projects/%s/repos/%s/ssh',
    this.v1KeysPrefix, projectKey, repoSlug);
  var body = {
...
Adapter.prototype.deleteDeployKey = function(projectKey, repoSlug, keyId,
  callback) {
  var url = util.format('%s/projects/%s/repos/%s/ssh/%s',
    this.v1KeysPrefix, projectKey, repoSlug, keyId);
  this.delete(url, callback);
};

Adapter.prototype.getCompareDiff = function (projectKey, repoSlug, newSha,
  oldSha, callback) {
  var url = util.format('%s/projects/%s/repos/%s/compare/changes?from=%s&to=%s',
    this.v1ApiPrefix, projectKey, repoSlug, newSha, oldSha);
  var self = this;
  ...
  function _onResponse(err, body) {
    ...
    if (_.has(body, 'values') && !_.isEmpty(body.values))
      responseObj.values = responseObj.values.concat(body.values);
    if (body.isLastPage === false && _.has(body, 'nextPageStart'))
