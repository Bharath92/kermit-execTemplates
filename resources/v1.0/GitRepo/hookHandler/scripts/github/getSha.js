...
var normalizeReleaseSha = require('./normalizeReleaseSha.js');
...
function getSha(params, callback) {
  ...
  async.series([
      ...
      _processReleaseWebhook.bind(null, bag),
...
function _processReleaseWebhook(bag, next) {
  if (bag.params.runType !== runTypes.WEBHOOK_RELEASE) return next();

  var who = bag.who + '|' + _processReleaseWebhook.name;
  console.log(who, 'Inside');

  normalizeReleaseSha(bag.params,
    function (err, sha) {
      if (err)
        return next(err);
