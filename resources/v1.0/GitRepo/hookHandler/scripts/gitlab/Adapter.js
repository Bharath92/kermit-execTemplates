...
Adapter.prototype.getCompareDiff = function (projectId, oldSha, newSha,
  callback) {
  var url = '/projects/' + projectId + '/repository/compare?from=' + oldSha +
    '&to=' + newSha;
  this.get(url, {}, callback);
};
...
// Deploy Keys
Adapter.prototype.getDeployKeys = function (projectFullName, callback) {
  var escapedProjectFullName = querystring.escape(projectFullName);
  var urlByVersion = {
    'v3': '/projects/' + escapedProjectFullName + '/keys',
    'v4': '/projects/' + escapedProjectFullName + '/deploy_keys'
  };
  var url = urlByVersion[this.gitlabVersion];
  this.get(url, {}, callback);
};

Adapter.prototype.postDeployKey = function (projectFullName, key, title,
  callback) {
  var escapedProjectFullName = querystring.escape(projectFullName);
  var urlByVersion = {
    'v3': '/projects/' + escapedProjectFullName + '/keys',
...
Adapter.prototype.deleteDeployKey = function (projectFullName, keyId,
  callback) {
  var escapedProjectFullName = querystring.escape(projectFullName);
  var urlByVersion = {
    'v3': '/projects/' + escapedProjectFullName + '/keys/' + keyId,
    'v4': '/projects/' + escapedProjectFullName + '/deploy_keys/' + keyId
  };
  var url = urlByVersion[this.gitlabVersion];
  this.del(url, callback);
};
...
//Tags
Adapter.prototype.getRepositoryTag = function (projectId, tagName,
  callback) {
  var url =
